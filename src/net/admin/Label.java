...
public class Label implements Serializable {
    ...
    public static Vector getLabelsExact(String sType, String sCode, String sText, String sLanguage, String sSort){
        PreparedStatement ps = null;
        ResultSet rs = null;
        ...
        String sAdd = "";
        if(sType.length() > 0){
            sAdd+= MedwanQuery.getInstance().getConfigParam("lowerCompare","OC_LABEL_TYPE")+" = ? AND ";
        }

        if(sCode.length() > 0){
            sAdd+= MedwanQuery.getInstance().getConfigParam("lowerCompare","OC_LABEL_ID")+" = ? AND ";
        }

        if(sLanguage.length() > 0){
            sAdd+= MedwanQuery.getInstance().getConfigParam("lowerCompare","OC_LABEL_LANGUAGE")+" = ? AND ";
        }

        if(sText.length() > 0){
            sAdd+= MedwanQuery.getInstance().getConfigParam("lowerCompare","OC_LABEL_VALUE")+" = ? AND ";
        }
