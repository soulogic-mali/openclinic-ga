...
public class Prescription extends OC_Object{
    ...
    public static Prescription get(String prescrUid){
        ...
        try{
            ...
            if(rs.next()){
                ...
                prescr.setRoute(rs.getString("OC_PRESCR_ROUTE"));
            }
            else{
        ...
        try{
            if(this.getUid().equals("-1") && !prescrWithSameDataExists){
                ...
                sSelect = "INSERT INTO OC_PRESCRIPTIONS (OC_PRESCR_SERVERID,OC_PRESCR_OBJECTID,OC_PRESCR_PATIENTUID,"+
                          ...
                          + "OC_PRESCR_DIAGNOSISUID,OC_PRESCR_ROUTE)"+
                          " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,?,?,?)";
                ...
                if(this.getTimeUnit()!=null && this.getTimeUnit().length() > 0) ps.setString(8,this.getTimeUnit());
                ...
                if(this.getSupplyingServiceUid()!=null && this.getSupplyingServiceUid().length() > 0) ps.setString(11,this.getSupplyingServiceUid());
            ...
            //If the already delivered quantity is lower than the prescribed quantity, we check if there are waiting records for this prescription in OC_DRUGSOUTLIST
            if(getRequiredPackages()-getDeliveredQuantity()>0) {
	            ps.close();
	            ps=oc_conn.prepareStatement("select * from OC_DRUGSOUTLIST where OC_LIST_PRESCRIPTIONUID=?");
	            ps.setString(1, getUid());
	            rs=ps.executeQuery();
	            if(rs.next()) {
	            	rs.close();
	            	ps.close();
	            	ps=oc_conn.prepareStatement("update OC_DRUGSOUTLIST set OC_LIST_QUANTITY=? where OC_LIST_PRESCRIPTIONUID=?");
	            	ps.setDouble(1, getRequiredPackages()-getDeliveredQuantity());
    ...
    public static Vector findUndelivered(Encounter encounter){
		String sSortCol="",sSortDir="";
    	Vector foundObjects = new Vector();
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try{
			// compose query
			String sSelect = "SELECT * FROM OC_PRESCRIPTIONS WHERE OC_PRESCR_PATIENTUID = ? AND OC_PRESCR_BEGIN >= ?"; 
			...
			ps.setString(1,encounter.getPatientUID());
    ...
    public static Vector find(Encounter encounter){
		String sSortCol="",sSortDir="";
    	Vector foundObjects = new Vector();
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try{
			...
			while(rs.next()){
				if(encounter.getEnd()!=null && rs.getDate("OC_PRESCR_BEGIN").after(encounter.getEnd())) {
					continue;
				}
				Prescription p = get(rs.getString("OC_PRESCR_SERVERID")+"."+rs.getString("OC_PRESCR_OBJECTID"));
				foundObjects.add(p);
    ...
    public static Vector findUndiagnosed(Encounter encounter){
		String sSortCol="",sSortDir="";
    	Vector foundObjects = new Vector();
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try{
			...
			while(rs.next()){
				Prescription p = get(rs.getString("OC_PRESCR_SERVERID")+"."+rs.getString("OC_PRESCR_OBJECTID"));
				if(ScreenHelper.checkString(p.getDiagnosisUid()).length()==0){
					foundObjects.add(p);
				}
			}
		}
    ...
    public double getDeliveredQuantity(){
        ...
        try{
		    ...
		    rs.close();
		    ps.close();
		    sQuery = "select sum(OC_OPERATION_UNITSCHANGED) as total"+
		            " from OC_PRODUCTSTOCKOPERATIONS, OC_PRODUCTSTOCKS "+
    ...
    public double getOpenQuantity() {
    	return getRequiredPackages()-getDeliveredQuantity()-getOrderlistQuantity();
    }
    
    public double getDeliveredQuantity(String encounteruid){
        double quantity = 0;
        return quantity;
    }

    public double getOrderlistQuantity(){
        double quantity = 0;
        PreparedStatement ps = null;
        ResultSet rs = null;
        Connection oc_conn = MedwanQuery.getInstance().getOpenclinicConnection();
        ...
        try{
            String sQuery = "select sum(OC_LIST_QUANTITY) as total"+
                            " from OC_DRUGSOUTLIST "+
                            "  where OC_LIST_PRESCRIPTIONUID=?";
            ps = oc_conn.prepareStatement(sQuery);
            ps.setString(1,getUid());
            rs = ps.executeQuery();
            if(rs.next()){
