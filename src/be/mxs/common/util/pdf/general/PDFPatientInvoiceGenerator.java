...
public class PDFPatientInvoiceGenerator extends PDFInvoiceGenerator {
    ...
    public ByteArrayOutputStream generatePDFDocumentBytes(final HttpServletRequest req, Invoice inv) throws Exception {
		...
		try{
			...
			// get specified invoice
			if(MedwanQuery.getInstance().getConfigInt("patientInvoiceShowFooter",1)==1){
            ...
            if(Pointer.getPointer("INV.SVC."+invoice.getUid()).length()>0) {
            	services.add(invoice.getUid());
            }
            else {
            ...
            if(services.size()>1 && MedwanQuery.getInstance().getConfigInt("defaultPatientInvoiceSeparatePagesPerService",0)==1){
	            int pages=0;
    ...
    private void addReceipt(PatientInvoice invoice) throws DocumentException {
        ...
        table.addCell(createTotalPriceCell(totalDebet,1,invoice.getDate(),sPaymentCurrency));
        ...
        cell=createTotalPriceCell(totalCredit,1,invoice.getDate(),sPaymentCurrency);
        ...
        table.addCell(createTotalPriceCell(invoice.getBalance(),1,invoice.getDate(),sPaymentCurrency));
        table.addCell(createEmptyCell(3));
        table.addCell(createValueCell(getTran("web","insurar"),1,8,Font.ITALIC));
        cell = new PdfPCell(new Paragraph(getTotalPriceString(totalinsurardebet,false,invoice.getDate(),sPaymentCurrency),FontFactory.getFont(FontFactory.HELVETICA,7,Font.ITALIC)));
        cell.setColspan(1);
        cell.setBorder(PdfPCell.NO_BORDER);
    ...
    private void addPatientData(){
        ...
        try {
            ...
            if(apc!=null){
                ...
                if(apc.quarter.length() > 0)   sContactData+= "\n"+apc.quarter;
    ...
    private PdfPTable getInsuranceData(Insurance insurance,PatientInvoice invoice){
        ...
        if((invoice.getMfpDoctor()!=null && invoice.getMfpDoctor().length()>0) || (invoice.getMfpDrugReceiver()!=null && invoice.getMfpDrugReceiver().length()>0) || (invoice.getMfpPost()!=null && invoice.getMfpPost().length()>0)){
        	if(MedwanQuery.getInstance().getConfigInt("hideCCBRTInvoiceFields",1)==0){
	            ...
	            table.addCell(createValueCell(":   "+(invoice.getMfpDoctor()==null?"":invoice.getMfpDoctor())+(invoice.getMfpDrugReceiver()!=null && invoice.getMfpDrugReceiver().length()>0?" ("+getTran("nhif.authorizationtypes",invoice.getMfpDrugReceiver())+")":""),5));
	            table.addCell(createLabelCell(getTran("web","ccbrt.comment"),2));
	            table.addCell(createValueCell(":   "+(invoice.getMfpPost()==null?"":invoice.getMfpPost()),5));
        	}
        }
    ...
    private void getDebets(PatientInvoice invoice,PdfPTable tableParent,Vector debets){
        ...
        if(MedwanQuery.getInstance().getConfigInt("defaultInvoiceSummarizeDebets",0)==1){
            ...
            for(int n=0;n<debets.size();n++){
            	...
            	if(debet.getPrestation()!=null && debet.getQuantity()!=0){
            		...
            		if(prestations.get(prestationClass+"."+debet.getPrestation().getDescription()+"."+debet.getInsuranceUid()+"."+debet.getPrestationUid())==null){
            			prestations.put(prestationClass+"."+debet.getPrestation().getDescription()+"."+debet.getInsuranceUid()+"."+debet.getPrestationUid(), debet);
            		}
            		else {
            			Debet olddebet = (Debet)prestations.get(prestationClass+"."+debet.getPrestation().getDescription()+"."+debet.getInsuranceUid()+"."+debet.getPrestationUid());
            			...
            			if(debet.getDate().after(olddebet.getDate())) {
        ...
        if(newdebets.size() > 0){
            ...
            // header
            if(MedwanQuery.getInstance().getConfigInt("defaultInvoiceSummarizeDebets",0)==1){
            	cell = createUnderlinedCell(getTran("web","prestationenddate"),1);
            }
            else {
            ...
            cell = createUnderlinedCell(getTran("system.manage","categorypatientshare"),1);
            cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
            singleCellHeaderTable = new PdfPTable(1);
            singleCellHeaderTable.addCell(cell);
            cell = createCell(new PdfPCell(singleCellHeaderTable),20,PdfPCell.ALIGN_CENTER,PdfPCell.NO_BORDER);
            ...
            for(int i=0; i<newdebets.size(); i++){
                ...
                if(!prestationClass.equalsIgnoreCase(activePrestationClass)){
                    ...
                    printPrestationClass(table,activePrestationClass,newdebets,invoice);
                }
            ...
            table.addCell(cell);
            table.addCell(createTotalPriceCell(totalPatient+totalInsurar+totalExtraInsurar,false,2,invoice.getDate(),sPaymentCurrency));
            table.addCell(createTotalPriceCell(totalPatient,false,2,invoice.getDate(),sPaymentCurrency));

            table.addCell(createTotalPriceCell(totalInsurar,false,2,invoice.getDate(),sPaymentCurrency));

            table.addCell(createTotalPriceCell(totalExtraInsurar,false,2,invoice.getDate(),sPaymentCurrency));
            cell = new PdfPCell(table);
            cell.setPadding(cellPadding);
            tableParent.addCell(createCell(cell,1,PdfPCell.ALIGN_LEFT,PdfPCell.NO_BORDER));
    ...
    private PdfPTable getCredits(PatientInvoice invoice){
        ...
        if(creditUids.size() > 0){
            ...
            cell.setPaddingRight(cellPadding);
            table.addCell(cell);
            table.addCell(createTotalPriceCell(total,false,3,invoice.getDate(),sPaymentCurrency));
            table.addCell(createEmptyCell(3));
    ...
    private void printPrestationClass(PdfPTable invoiceTable,String prestationClass,Vector debets, PatientInvoice invoice){
        ...
        cell = new PdfPCell(new Paragraph(getTotalPriceString(classPatientAmount+classInsurarAmount+classExtraInsurarAmount,false,invoice.getDate(),sPaymentCurrency),FontFactory.getFont(FontFactory.HELVETICA,7,Font.BOLDITALIC)));
        cell.setColspan(20);
        cell.setBorder(PdfPCell.BOX);
        cell.setVerticalAlignment(PdfPCell.ALIGN_TOP);
        cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
        invoiceTable.addCell(cell);
        cell = new PdfPCell(new Paragraph(getTotalPriceString(classPatientAmount,false,invoice.getDate(),sPaymentCurrency),FontFactory.getFont(FontFactory.HELVETICA,7,Font.BOLDITALIC)));
        cell.setColspan(20);
        cell.setBorder(PdfPCell.BOX);
        cell.setVerticalAlignment(PdfPCell.ALIGN_TOP);
        cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
        invoiceTable.addCell(cell);
        cell = new PdfPCell(new Paragraph(getTotalPriceString(classInsurarAmount,false,invoice.getDate(),sPaymentCurrency),FontFactory.getFont(FontFactory.HELVETICA,7,Font.BOLDITALIC)));
        cell.setColspan(20);
        cell.setBorder(PdfPCell.BOX);
        cell.setVerticalAlignment(PdfPCell.ALIGN_TOP);
        cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
        invoiceTable.addCell(cell);
        cell = new PdfPCell(new Paragraph(getTotalPriceString(classExtraInsurarAmount,false,invoice.getDate(),sPaymentCurrency),FontFactory.getFont(FontFactory.HELVETICA,7,Font.BOLDITALIC)));
        cell.setColspan(20);
        cell.setBorder(PdfPCell.BOX);
        cell.setVerticalAlignment(PdfPCell.ALIGN_TOP);
        cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
    ...
    private void printDebet(PdfPTable invoiceTable, Debet debet){
        ...
        if(prestation!=null){
            ...
            if(MedwanQuery.getInstance().getConfigInt("enableCommentsForDebetsDefaultInvoice",1)==1) {
            	if(checkString(debet.getComment()).length()>0) {
            		sPrestationDescr+=" ("+debet.getComment()+")";
            	}
            }
        }
        ...
        // row
        if(MedwanQuery.getInstance().getConfigInt("defaultInvoiceSummarizeDebets",0)==1){
        	invoiceTable.addCell(createValueCell(sDebetDate,30));
        }
        else {
        ...
        cell.setHorizontalAlignment(PdfPCell.ALIGN_RIGHT);
        invoiceTable.addCell(cell);
        invoiceTable.addCell(createTotalPriceCell((debetAmountPatient+debetAmountInsurar+debetAmountExtraInsurar)/debet.getQuantity(),false,20,debet.getDate(),sPaymentCurrency));
        invoiceTable.addCell(createTotalPriceCell((debetAmountPatient+debetAmountInsurar+debetAmountExtraInsurar),false,20,debet.getDate(),sPaymentCurrency));
        invoiceTable.addCell(createTotalPriceCell(debetAmountPatient,false,20,debet.getDate(),sPaymentCurrency));
        invoiceTable.addCell(createTotalPriceCell(debetAmountInsurar,false,20,debet.getDate(),sPaymentCurrency));
        invoiceTable.addCell(createTotalPriceCell(debetAmountExtraInsurar,false,20,debet.getDate(),sPaymentCurrency));
    }

    //--- PRINT CREDIT (payment) ------------------------------------------------------------------
    private void printCredit(PdfPTable invoiceTable, PatientCredit credit){
        String sCreditDate = new SimpleDateFormat("d/M/yy").format(credit.getDate());
    ...
    private PdfPTable getSaldo(Vector debets, PatientInvoice invoice){
        ...
        table.addCell(cell);
        table.addCell(createTotalPriceCell(this.patientDebetTotal,3,invoice.getDate(),sPaymentCurrency));
        table.addCell(createEmptyCell(3));
        ...
        cell.setPaddingRight(5);
        table.addCell(cell);
        table.addCell(createTotalPriceCell(this.creditTotal,(this.creditTotal>=0),3,invoice.getDate(),sPaymentCurrency));
        table.addCell(createEmptyCell(3));
        ...
        table.addCell(createTotalPriceCell(saldo,3,invoice.getDate(),sPaymentCurrency));
        table.addCell(createEmptyCell(3));
        ...
        cell.setPaddingRight(5);
        table.addCell(cell);
        table.addCell(createTotalPriceCell(complementaryinsurar,3,invoice.getDate(),sPaymentCurrency));
        table.addCell(createEmptyCell(3));
