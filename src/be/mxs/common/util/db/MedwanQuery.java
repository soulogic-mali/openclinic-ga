...
public class MedwanQuery {
	...
	//--- CONSTRUCTOR -----------------------------------------------------------------------------
    public MedwanQuery(String uri, boolean bStartThreads){
    	try {
        ...
        stdDateFormat = new SimpleDateFormat(getConfigString("euDateFormat","dd/MM/yyyy"));
        this.uri = uri;
        ...
        if(bStartThreads) {
	        if(scheduler==null || (!scheduler.isActive() && !scheduler.isStarted())){
	        	if(scheduler!=null) {
    ...
    public static ScanDirectoryMonitor getScanDirectoryMonitor() {
		return scanDirectoryMonitor;
	}

	public static void setScanDirectoryMonitor(ScanDirectoryMonitor scanDirectoryMonitor) {
		MedwanQuery.scanDirectoryMonitor = scanDirectoryMonitor;
	}
    ...
    public TransactionVO getTransactionForItem(String type, String value){
        ...
        try{
            ...
            String sQuery = "select i.*,t.ts from Transactions t, Items i where t.serverid=i.serverid and t.transactionid=i.transactionid and valuehash=? and type=? and value=? order by ts desc";
    ...
    public Vector getLastTransactions(String eligibletypes, String requiredtypes) {
    	Vector transactions=new Vector();
    	Connection conn = getOpenclinicConnection();
        try{
            conn = getOpenclinicConnection();
            String et = "'unknown'";
            String[] etypes = eligibletypes.split(";");
            for(int n=0;n<etypes.length;n++) {
            	et+=",'"+etypes[n]+"'";
            }
            String rt = "'unknown'";
            String[] rtypes = requiredtypes.split(";");
            for(int n=0;n<rtypes.length;n++) {
            	rt+=",'"+rtypes[n]+"'";
            }
            String sQuery=	"select t.serverid,t.transactionid from transactions t,("+
    				" select max(transactionid) transactionid,healthrecordid from transactions where transactiontype in ("+et+")"+
    				" group by healthrecordid) a"+
    				" where t.transactionid=a.transactionid and t.healthrecordid=a.healthrecordid and transactiontype in ("+rt+")";
    ...
    public static void reload(){
        ...
        medwanQuery = new MedwanQuery(medwanQuery.uri,true);
    ...
    public static MedwanQuery getInstance(){
        if(medwanQuery == null){
            medwanQuery = new MedwanQuery(null,true);
        }
        return medwanQuery;
    }
    
    public static MedwanQuery getInstance(boolean bStartThreads){
        if(medwanQuery == null || !ImageIO.getImageReadersByFormatName("DICOM").hasNext()){
            medwanQuery = new MedwanQuery(null,bStartThreads);
        }
        return medwanQuery;
    }
    
    public static MedwanQuery getInstance(String uri, boolean bStartThreads){
        if(medwanQuery == null){
            medwanQuery = new MedwanQuery(uri,bStartThreads);
        }
    ...
    public static MedwanQuery getInstance(String uri){
        if(medwanQuery == null){
            medwanQuery = new MedwanQuery(uri,true);
        }
        else if((medwanQuery.uri == null && uri!=null) || (medwanQuery.uri!=null && !medwanQuery.uri.equalsIgnoreCase(uri))){
            Hashtable labels = medwanQuery.labels;
            medwanQuery = new MedwanQuery(uri,true);
        ...
        try{
            ...
            for (Occuprs = Occupstatement.executeQuery(); Occuprs.next();){
                ...
                if(transactionVO.getServerId()>1) {
                	//Load Items that are important for external transations
                    items.add(getItem(serverId, transactionId, "be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_REFERRAL_SOURCESITE"));
                    items.add(getItem(serverId, transactionId, "be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_REFERRAL_USER"));
                }
  ...
  //--- GET AGE ---------------------------------------------------------------------------------
    ...
    public ItemVO getItem(int serverId, int transactionId, String itemType){
        ...
        try{
            ...
            if(Occuprs.next()){
                ...
                itemVO = new ItemVO(new Integer(Occuprs.getInt("itemId")), Occuprs.getString("type"), Occuprs.getString("value").replaceAll("\\{semicolon\\}", ";"), Occuprs.getDate("date"), itemContextVO);
            }
            Occuprs.close();
            Occupstatement.close();
            OccupdbConnection.close();
    ...
    //--- GET ITEM --------------------------------------------------------------------------------
    public Vector getItemsLike(int serverId, int transactionId, String itemType){
        Vector items = new Vector();
    	ItemVO itemVO = null;
        Connection OccupdbConnection;
        try{
            OccupdbConnection = getOpenclinicConnection();
            PreparedStatement Occupstatement = OccupdbConnection.prepareStatement("SELECT * from Items where serverid=? and transactionId=? and type like ?");
            Occupstatement.setInt(1, serverId);
            Occupstatement.setInt(2, transactionId);
            Occupstatement.setString(3, itemType+"%");
            ResultSet Occuprs = Occupstatement.executeQuery();
    ...
    public double getMaximumItemVO(int personId, String itemType){
        ItemVO itemVO = null;
        Connection OccupdbConnection;
        double d = -99999999;
        try{
            OccupdbConnection = getOpenclinicConnection();
            String sSql="SELECT * from Transactions a,Items b, Healthrecord c where a.healthRecordId=c.healthRecordId and c.personId=? and a.transactionId=b.transactionId and a.serverid=b.serverid and b.type in ("+itemType+")";
            PreparedStatement Occupstatement = OccupdbConnection.prepareStatement(sSql);
            Occupstatement.setInt(1, personId);
            ResultSet Occuprs = Occupstatement.executeQuery();
            while(Occuprs.next()){
            	try {
            		double dValue=Double.parseDouble(Occuprs.getString("value").replaceAll(",","."));
            		if(dValue>d) {
            			d=dValue;
            		}
            	}
    ...
    public double getMinimumItemVO(int personId, String itemType){
        ItemVO itemVO = null;
        Connection OccupdbConnection;
        double d = 99999999;
        try{
    ...
    public double getMaximumItemValue(int personId, String itemType){
        double d = getMaximumItemVO(personId, itemType);
        return d;
    }
    public double getMinimumItemValue(int personId, String itemType){
        double d = getMinimumItemVO(personId, itemType);
        return d;
    }
    ...
    public TransactionVO getLastModifiedTransactionByType(int personId, String transactionType){
        TransactionVO transactionVO = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        Connection conn=getOpenclinicConnection();
    ...
    public TransactionVO loadTransaction(String uid) {
    	if(uid.split("\\.").length<2) {
    		return null;
    	}
    	return loadTransaction(Integer.parseInt(uid.split("\\.")[0]),Integer.parseInt(uid.split("\\.")[1]));
    }
...
//                var = "";
    ...
    public TransactionVO updateTransaction(int personid, TransactionVO transactionVO){
        try{
            ...
            ps.setInt(2, transactionVO.getTransactionId());
            ResultSet rs = ps.executeQuery();
            int healthRecordId = 0;
            if(rs.next()){
    ...
    public Vector getItemHistory(int healthRecordId, String itemType){
        ...
        try{
            ...
            ps.setInt(1, healthRecordId);
            ResultSet rs = ps.executeQuery();
            ItemVO historyItem = null;
            int n = -1,serverid=0,itemid=-1;
            ...
            while (rs.next()){
            	...
            	if(transactionid!=activetransactionid) {
            		//Dit is een nieuw item, bewaar het vorige en creëer een nieuw
            		if(activetransactionid>-1) {
                        historyItem = new ItemVO(itemid, itemType, value, updatetime, null);
    ...
    public Vector<TransactionVO> getTransactionsByType(int personId, String transactionType){
    ...
    public Vector<TransactionVO> getTransactionsByTypeBetween(int personId, String transactionType,java.util.Date begin, java.util.Date end){
        Vector<TransactionVO> transactionList = new Vector<TransactionVO>();
        Connection OccupdbConnection;
        String sSql = "SELECT a.* FROM Transactions a, Healthrecord b"+
                      " WHERE a.healthRecordId = b.healthRecordId"+
        		      "  AND personId = ?"+
        ...
        try{
            ...
            Occupstatement.setInt(1, personId);
            Occupstatement.setString(2, transactionType);
            Occupstatement.setTimestamp(3, new java.sql.Timestamp(begin.getTime()));
            Occupstatement.setTimestamp(4, new java.sql.Timestamp(end.getTime()));
            ResultSet Occuprs;
            for(Occuprs = Occupstatement.executeQuery(); Occuprs.next();){
                transactionList.add(loadTransaction(Occuprs.getInt("serverid"),Occuprs.getInt("transactionId")));
            }
    ...
    //--- GET ITEMS VALUES FROM ITEMS BY TYPE -----------------------------------------------------
    public Vector<TransactionVO> getTransactionsForItem(String itemType, String itemValue){
        Vector values = new Vector();
        PreparedStatement ps = null;
        ResultSet rs = null;
        
        String sSelect = "select distinct serverid,transactionid from Items where type= ? and value=?";
        Connection oc_conn=MedwanQuery.getInstance().getOpenclinicConnection();
        try{
            ps = oc_conn.prepareStatement(sSelect);
            ps.setString(1, itemType);
            ...
            // run thru found values
            while (rs.next()){
                values.add(loadTransaction(rs.getInt("serverid"), rs.getInt("transactionid")));
            }
        }
        catch(SQLException sqle){
            sqle.printStackTrace();
    ...
    public String setSynchroniseCounters(String name, String table, String field, String bd){
        ...
        try{
        	for(int n=0;n<table.split(";").length;n++) {
        		iRealValue = Math.max(getIntTotalTable(table.split(";")[n], field.split(";")[n], bd),iRealValue);
        	}
        }
        catch(Exception e){
