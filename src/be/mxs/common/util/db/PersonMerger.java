...
public class PersonMerger {
    ...
    public void merge(int keepPersonId, int removePersonId, boolean addPersonsToMergePersons) throws Exception {
        ...
        // update Users
        ps = adminConn.prepareStatement("Update users set personid=? WHERE personid = ?");
        ...
        //*** STEP 7.17 : UPDATE OC_POINTERS *****************************************************
        if(Debug.enabled) Debug.println("*** STEP 7.16 ***");
        sQuery = "UPDATE OC_POINTERS SET OC_WICKET_DEBET_REFERENCEUID = ? WHERE OC_WICKET_DEBET_REFERENCETYPE='Person' and OC_WICKET_DEBET_REFERENCEUID = ?";
        ps = occupConn.prepareStatement(sQuery);
        ps.setString(1,keepPersonId+"");
        ps.setString(2,removePersonId+"");
        //updates = ps.executeUpdate();
        ...
        //*** STEP 7.18 : UPDATE RequestedLabanalyses *****************************************************
        if(Debug.enabled) Debug.println("*** STEP 7.18 ***");
        sQuery = "UPDATE RequestedLabanalyses SET patientid = ? WHERE patientid = ?";
        ps = occupConn.prepareStatement(sQuery);
        ps.setString(1,keepPersonId+"");
        ps.setString(2,removePersonId+"");
        updates = ps.executeUpdate();
    ...
    public void update(int keepPersonId, int removePersonId, boolean addPersonsToMergePersons) throws Exception {
        adminConn = MedwanQuery.getInstance().getAdminConnection();
        occupConn = MedwanQuery.getInstance().getOpenclinicConnection();
        int updates;
        ...
        // delete source person from Admin
        ps = adminConn.prepareStatement("update Admin set personid=? WHERE personid = ?");
        ps.setInt(1,keepPersonId);
        ps.setInt(2,removePersonId);
        ps.executeUpdate();
        ps.close();
        
        //Make sure to update counters if needed
		if(MedwanQuery.getInstance().getOpenclinicCounterNoIncrement("PersonID")<keepPersonId){
			MedwanQuery.getInstance().getOpenclinicCounterNoOffline("PersonID", keepPersonId);
		}
