...
<%
    String sGender = "&nbsp;", sComment = "&nbsp;", sNativeCountry = "&nbsp;", sLanguage = "&nbsp;", sNatreg = "&nbsp;", sVip="0"
            ,sNativeTown="", sCivilStatus = "&nbsp;", sTracnetID = "&nbsp;", sTreatingPhysician = "&nbsp;", sComment3="", sDeathCertificateTo="", sDeathCertificateOn="", sExport="0", sLastExport="0";
    ...
    if ((activePatient.nativeTown!=null)&&(activePatient.nativeTown.trim().length()>0)) {
        sNativeTown = activePatient.nativeTown;
    }
...
<table width='100%' cellspacing="1" class="list">
	<%
		int nBloodGifts = MedwanQuery.getInstance().getTransactionsByType(Integer.parseInt(activePatient.personid), "be.mxs.common.model.vo.healthrecord.IConstants.TRANSACTION_TYPE_CNTS_BLOODGIFT").size();
		String sOld=Pointer.getPointer("oldblooddonations."+activePatient.personid,"0");
		try{
			nBloodGifts+=Integer.parseInt(sOld);
		}
		catch(Exception e){
			e.printStackTrace();
		}
	...
	+setRow(request,"Web","comment",sComment,sWebLanguage)
        +setRow(request,"Web","numberofbloodgifts",nBloodGifts+"",sWebLanguage)
