...
<%
	String accessright="mspls.registry.ssn";
%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1">
        ...
        <tr>
            ...
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultNumericInput(session, (TransactionVO) transaction, "ITEM_TYPE_WEIGHT", 5, 2, 150, sWebLanguage,"calculateWeightOverHeight();") %> kg
            </td>
        </tr>
        <tr>
            <td class="admin"><%=getTran(request,"web", "height", sWebLanguage)%></td>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultNumericInput(session, (TransactionVO) transaction, "ITEM_TYPE_HEIGHT", 5, 20, 250, sWebLanguage,"calculateWeightOverHeight();") %> cm
            </td>
        </tr>
        <tr>
            <td class="admin"><%=getTran(request,"web", "weightoverlength", sWebLanguage)%></td>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultTextInputReadonly(session, (TransactionVO)transaction, "ITEM_TYPE_WEIGHTOVERHEIGHT", 5) %>
            </td>
        </tr>
        <tr>
            <td class="admin"><%=getTran(request,"web", "weightoverlengthz", sWebLanguage)%></td>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultTextInputReadonly(session, (TransactionVO)transaction, "ITEM_TYPE_WEIGHTOVERHEIGHTZ", 5) %>
            </td>
        </tr>
        <tr>
            <td class="admin"><%=getTran(request,"mspls", "admissioncriteria", sWebLanguage)%></td>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultRadioButtons((TransactionVO)transaction, request, "mspls.ssn.admissioncriteria",  "ITEM_TYPE_ADMISSIONCRITERIA", sWebLanguage, true,"","") %>
            </td>
        </tr>
        ...
        <% } %>
        <tr>
            <td class="admin"><%=getTran(request,"mspls", "hivrelated", sWebLanguage)%></td>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultCheckBoxes((TransactionVO)transaction, request, "mspls.ssn.hivrelated",  "ITEM_TYPE_HIVRELATED", sWebLanguage, true) %>
            </td>
        </tr>
        <tr>
...
<script>
  function calculateWeightOverHeight(){
	  if(document.getElementById("ITEM_TYPE_DISCHARGEWEIGHT").value.trim()=="" || document.getElementById("ITEM_TYPE_DISCHARGEHEIGHT").value.trim()==""){
		  document.getElementById("ITEM_TYPE_DISCHARGEWEIGHTOVERHEIGHT").value="";
	  }
	  else{
		  document.getElementById("ITEM_TYPE_DISCHARGEWEIGHTOVERHEIGHT").value=(document.getElementById("ITEM_TYPE_DISCHARGEWEIGHT").value/document.getElementById("ITEM_TYPE_DISCHARGEHEIGHT").value).toFixed(2);
		  checkWeightForHeight(document.getElementById("ITEM_TYPE_DISCHARGEHEIGHT").value,document.getElementById("ITEM_TYPE_DISCHARGEWEIGHT").value,"ITEM_TYPE_DISCHARGEWEIGHTOVERHEIGHTZ");
	  }
  ...
  function checkAdmissionCriteria(){
	  if(!(document.getElementById("ITEM_TYPE_BRACHIALCIRCUMFERENCE").value.trim()=="") && document.getElementById("ITEM_TYPE_BRACHIALCIRCUMFERENCE").value*1<210){
		  document.getElementById("ITEM_TYPE_ADMISSIONCRITERIA.2").checked=true;
	  }
	  else{
		  document.getElementById("ITEM_TYPE_ADMISSIONCRITERIA.2").checked=false;
	  }
	  updateMultiCheckbox(document.getElementById("ITEM_TYPE_ADMISSIONCRITERIA.2"),"ITEM_TYPE_ADMISSIONCRITERIA","2");
  }
