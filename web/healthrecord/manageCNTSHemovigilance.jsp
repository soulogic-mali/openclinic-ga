...
<%
	String accessright="cnts.hemovigilance";
%>
...
%>
<%
	if(activePatient==null){activePatient=new AdminPerson();}
%>
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1" cellpadding='0'>
        ...
        <tr>
        	<td class='admin'><%=getTran(request,"web","bloodgroup.receiver",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultSelect(request, (TransactionVO)transaction, "ITEM_TYPE_BLOODGROUP_RECEIVER", "abobloodgroup", sWebLanguage, "") %></td>
        	<td class='admin'><%=getTran(request,"web","pocketnumber",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultTextInput(session, (TransactionVO)transaction, "ITEM_TYPE_BLOODPOCKET_NUMBER1", 20) %></td>
        </tr>
        <tr>
        	<td class='admin'><%=getTran(request,"web","transfusionreasons",sWebLanguage) %></td>
        	<td class='admin2' colspan='3'><%=SH.writeDefaultCheckBoxes((TransactionVO)transaction, request, "bloodtransfusion.reason", "ITEM_TYPE_BLOODTRANSFUSION_REASON", sWebLanguage, true) %></td>
        </tr>
        <tr>
        	<td class='admin'><%=getTran(request,"web","transfusionduration",sWebLanguage) %></td>
        	<td class='admin2'>
				<select class='text' name='currentTransactionVO.items.<ItemVO[hashCode=<%=((TransactionVO)transaction).getItem("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_TRANSFUSIONHOURS").getItemId() %>]>.value'">
					<%
						for(int n=0;n<24;n++){
							out.println("<option value='"+n+"' "+(((TransactionVO)transaction).getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_TRANSFUSIONHOURS").equalsIgnoreCase(n+"")?"selected":"")+">"+n+"</option>");
						}
					%>
				</select> <%=getTran(request,"web","hours",sWebLanguage) %>
				<select class='text' name='currentTransactionVO.items.<ItemVO[hashCode=<%=((TransactionVO)transaction).getItem("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_TRANSFUSIONMINUTES").getItemId() %>]>.value'">
					<%
						for(int n=0;n<60;n++){
							out.println("<option value='"+n+"' "+(((TransactionVO)transaction).getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_TRANSFUSIONMINUTES").equalsIgnoreCase(n+"")?"selected":"")+">"+n+"</option>");
						}
					%>
				</select> <%=getTran(request,"web","minutes",sWebLanguage) %>
			</td>
        	<td class='admin'><%=getTran(request,"web","technicalissues",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_BLOODTRANSFUSION_TECHNICALISSUES", 60, 2) %></td>
        </tr>
        <tr class='admin'><td colspan='4'><%=getTran(request,"web","receiver.reactions",sWebLanguage) %></td></tr>
        <tr>
        	<td class='admin'><%=getTran(request,"web","transfusion.wellsupported",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultRadioButtons((TransactionVO)transaction, request, "yesno", "ITEM_TYPE_BLOODTRANSFUSION_WELLSUPPORTED", sWebLanguage, true, "onchange='checkSupported()'", "") %></td>
        	<td class='admin'><span id='effects1'><%=getTran(request,"web","undisred.effects",sWebLanguage) %></span></td>
        	<td class='admin2'><span id='effects2'><%=SH.writeDefaultCheckBoxes((TransactionVO)transaction, request, "bloodtransfusion.undesiredeffects", "ITEM_TYPE_BLOODTRANSFUSION_UNDESIREDEFFECTS", sWebLanguage, true) %></span></td>
        </tr>
        <tr>
        	<td class='admin'><%=getTran(request,"web","physician",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultTextInput(session, (TransactionVO)transaction, "ITEM_TYPE_BLOODTRANSFUSION_PHYSICIAN", 30) %></td>
        	<td class='admin'><%=getTran(request,"web","otherreactions",sWebLanguage) %></td>
        	<td class='admin2'><%=SH.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_BLOODTRANSFUSION_OTHERREACTIONS", 60, 2) %></td>
        </tr>
    </table>
...
<script>  
  function checkSupported(){
	  if(document.getElementById("ITEM_TYPE_BLOODTRANSFUSION_WELLSUPPORTED").value=="0;"){
			 document.getElementById("effects1").style.display=""; 
			 document.getElementById("effects2").style.display=""; 
	  }
	  else{
			 document.getElementById("effects1").style.display="none"; 
			 document.getElementById("effects2").style.display="none"; 
	  }
  }
