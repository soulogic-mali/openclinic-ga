...
<%
	String accessright="mspls.registry.fp";
%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1">
        ...
        <tr>
            ...
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultRadioButtons((TransactionVO)transaction, request, "mspls.pf.methods",  "ITEM_TYPE_FP_METHODNEW", sWebLanguage, true, "", " ") %>
            </td>
        </tr>
        <tr>
            <td class="admin"><%=getTran(request,"web", "acceptant", sWebLanguage)%></td>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultRadioButtons((TransactionVO)transaction, request, "mspls.pf.acceptant",  "ITEM_TYPE_FP_ACCEPTANT", sWebLanguage, true, "", " ") %>
            </td>
        </tr>
        <tr class='admin'>
        ...
        <tr>
            ...
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultTextArea(session, (TransactionVO) transaction, "ITEM_TYPE_REASONFORCHANGE", 60,1) %>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultTextArea(session, (TransactionVO) transaction, "ITEM_TYPE_FP_CONSEQUENCEOTHER", 60,1) %>
            </td>
        </tr>
        <tr>
            <td class="admin"><%=getTran(request,"web", "newproduct", sWebLanguage)%></td>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultSelect(request, (TransactionVO)transaction, "ITEM_TYPE_FP_NEWPRODUCTTYPE","mspls.fp.producttype", sWebLanguage, "") %>
            	<%=getTran(request,"web","quantity",sWebLanguage) %>: <%=ScreenHelper.writeDefaultNumericInput(session, (TransactionVO) transaction, "ITEM_TYPE_FP_QUANTITY", 5, 0, 1000, sWebLanguage) %><br/>
            	<%=ScreenHelper.writeDefaultTextArea(session, (TransactionVO) transaction, "ITEM_TYPE_FP_NEWPRODUCT", 60,1) %>
            </td>
        </tr>
        <tr>
        	<td colspan='2'><hr/></td>
