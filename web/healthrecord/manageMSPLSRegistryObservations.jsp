...
<%
	String accessright="mspls.registry.observations";
%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1">
        ...
        <tr>
            <td class="admin" rowspan="2"><%=getTran(request,"mspls", "reasonforobservation", sWebLanguage)%></td>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultSelect(request, (TransactionVO)transaction, "ITEM_TYPE_REASON", "mspls.observation.reason", sWebLanguage, "") %>
            </td>
        </tr>
        <tr>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_REASONOTHER", 60, 1) %>
            </td>
        </tr>
        <tr>
            <td class="admin"><%=getTran(request,"mspls", "maintreatment", sWebLanguage)%></td>
            <td class="admin2">
            	<%=ScreenHelper.writeDefaultTextArea(session, (TransactionVO)transaction, "ITEM_TYPE_MAINTREATMENT", 60, 1) %>
            </td>
        </tr>
        <tr>
            <td class="admin"><%=getTran(request,"mspls", "exitdiagnosis", sWebLanguage)%></td>
            <td class="admin2">
				<table width='100%'>
					<tr>
        ...
        <tr>
            <td class="admin"><%=getTran(request,"mspls", "referencereason", sWebLanguage)%></td>
            <td class="admin2">
