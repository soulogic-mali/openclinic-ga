            ...
            %>
						...
						if(prescription==null){
							%>			                    
			                    ...
			                    <tr id='prescriptionline'>
			                        ...
			                        <td class="admin2">
			                        	<select class='text' name="EditPrescriptionUid" id="EditPrescriptionUid">
			                        	<%
			                        		//Select existing open prescription
											if(sEditPrescriptionUid.length()>0){
												Prescription p = Prescription.get(sEditPrescriptionUid);
		                        				out.println("<option value='"+p.getUid()+"'>["+p.getUid()+"] "+p.getBegin()+" / "+User.getFullUserName(p.getUpdateUser())+"</option>");
											}
											else if(activePatient!=null){
												out.println("<option value=''></option>");
				                        		ProductStock productStock = ProductStock.get(sEditProductStockUid);
				                        		if(productStock!=null){
					                        		Vector prescriptions = Prescription.getActivePrescriptions(activePatient.personid);
					                        		for(int n=0;n<prescriptions.size();n++){
					                        			Prescription p = (Prescription)prescriptions.elementAt(n);
					                        			if(p.getProductUid().equalsIgnoreCase(productStock.getProductUid()) && p.getRequiredPackages()-p.getDeliveredQuantity()>0){
					                        				out.println("<option value='"+p.getUid()+"'>["+p.getUid()+"] "+p.getBegin()+" / "+User.getFullUserName(p.getUpdateUser())+"</option>");
					                        			}
					                        		}
				                        		}
                ...
                <script>
                  ...
                  function displaySrcDestSelector(){
                	if(document.getElementById('EditOperationDescr') && '<%=MedwanQuery.getInstance().getConfigString("productstockoperationsonexpireddrugs","medicationdelivery.4,medicationdelivery.5,")%>'.indexOf(document.getElementById('EditOperationDescr').value)<0 && document.getElementById("saveButton")){
                  	  if(availableLevel<=0){
	            		  //Hide save button
...
<script>
  ...
  function checkStockFields(){
    ...
    if(maySubmit==false){
      alert('<%=getTran(request,"web.manage","dataMissing",sWebLanguage)%>');
    }
