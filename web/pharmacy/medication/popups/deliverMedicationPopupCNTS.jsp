...
<form name='transactionForm' method='post'>
	...
	<table width='100%'>
		...
		<tr>
			...
			<td class='admin2'>
				<select class='text' name='destination' id='destination' onchange='countSelected()'>
					<option/>
					<%
						Vector<ServiceStock> destinations = ServiceStock.findAll();
						for(int n=0;n<destinations.size();n++){
							ServiceStock serviceStock = destinations.elementAt(n);
							if(serviceStock.getUid().equalsIgnoreCase(productStock.getServiceStockUid())){
								continue;
							}
							if(!serviceStock.isReceivingUser(activeUser.userid)){
								continue;
							}
							out.println("<option value='"+serviceStock.getUid()+"'>"+serviceStock.getName()+"</option>");
						}
					%>
				</select>
			</td>
		...
		<tr>
			<td class='admin' nowrap><%=getTran(request,"Web","availablequantity",sWebLanguage)%>&nbsp;</td>
			<td class='admin2'><%=productStock.getAvailableLevel() %></td>
		</tr>
		<tr>
...
<script>
	function countSelected(){
		...
		if(nChecked><%=productStock.getAvailableLevel()%>){
			document.getElementById('submitButton').style.display='none';
		}
		return nChecked;
	}
