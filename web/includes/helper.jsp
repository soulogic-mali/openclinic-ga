...
<%!
    ...
    public static String getProjectImage(String sImage) {
    	String s = sImage;
    	String sProject = SH.cs("defaultProject","oc");
    	String sLocalProjectPath = SH.cs("localProjectPath","");
    	if(new File(sLocalProjectPath+"/projects/"+sProject+"/"+sImage).exists()){
    		return "/projects/"+sProject+"/"+sImage;
    	}
    	return s;
    }
    ...
    public String c(String sString){
        return ScreenHelper.checkString(sString);
    }
    ...
    public String getConfigStringDB(String key, java.sql.Connection connection){
        return checkString(Config.getNotDeletedConfigByKey(key));
    }

    //--- SET CONFIG STRING DB --------------------------------------------------------------------
    public void setConfigStringDB(String key, String value, java.sql.Connection connection){
    ...
    public String contextHeader(HttpServletRequest request, String language, boolean setBackGround, String subTitle){
        String result = "<table width='100%' cellspacing='0' id='contextHeader' class='menu' style='border-bottom:none;'>";
        ...
        try{
            ...
            if(transactionVO!=null){
				...
				else if(transactionVO.getServerId()>1 && transactionVO.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_REFERRAL_SOURCESITE").length()>0){
					result+=" <span id='linkedservice'>["+transactionVO.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_REFERRAL_SOURCESITE")+"]</span>";
				}
            }
            result+= "</td>";
            ...
            if(sessionContainerWO.getUserVO().userId.intValue()==transactionVO.getUser().userId.intValue()){
            	result+= "<input id='privatetransaction' name='privatetransaction' class='hand' style='vertical-align: middle' type='checkbox' value='1' "+(checkString(transactionVO.getItemValue("be.mxs.common.model.vo.healthrecord.IConstants.ITEM_TYPE_PRIVATETRANSACTION")).equalsIgnoreCase("1")?"checked":"")+"/><span style='vertical-align: middle'>"+getLabel(null,"web","privatetransaction",language,"privatetransaction")+"</span>";
            }
            else{
    ...
    public String checkPermissionPopup(String sScreen, String sPermission, User activeUser){
        String s= ScreenHelper.checkPermission(sScreen,sPermission,activeUser,true,sCONTEXTPATH);
        if(s.length()>0){
        	s+=getTranNoLink("web","nopermission",activeUser==null || activeUser.person==null?"en":activeUser.person.language)+"<div style='display: none'>";
        }
        return s;
    ...
    //--- CHECK PERMISSION POPUP (screen is a popup) ----------------------------------------------
    public String checkPermissionPopup(JspWriter out,String sScreen, String sPermission, User activeUser){
        String s= ScreenHelper.checkPermission(sScreen,sPermission,activeUser,true,sCONTEXTPATH);
        if(s.length()>0){
        	s+=getTranNoLink("web","nopermission",activeUser==null || activeUser.person==null?"en":activeUser.person.language)+"<div style='display: none'>";
            try{
    ...
    public String takeOverTransaction(SessionContainerWO sessionContainerWO, User activeUser, String sFunction){
        ...
        if(tran!=null){
            if((tran.getTransactionId().intValue() > 0) && (!activeUser.userid.equals(tran.getUser().getUserId().intValue()+""))){
            	...
            	User tranUser = User.get(tran.getUser().getUserId());
            	if(tranUser.getParameter("finalclinicaldata").equalsIgnoreCase("on")){
            		sReturn = "alert('"+ScreenHelper.getTranNoLink("web","cannotmodifyfinaluserdata",sWebLanguage)+" ["+tranUser.person.getFullName()+"]')";
            	}
            	else if(tranUser.getParameter("finalauthor").equalsIgnoreCase("on")){
            		sReturn = sFunction;
            	}
            	else{
    ...
    public String getButtonsHtml(HttpServletRequest req, User activeUser, AdminPerson activePatient,
        ...
        // CREATE PDF
           ...
           .append("}");
	        html.append("function doReferral(){")
            .append("var tranID = '").append(checkString(req.getParameter("be.mxs.healthrecord.transaction_id"))).append("';")
            .append("var serverID = '").append(checkString(req.getParameter("be.mxs.healthrecord.server_id"))).append("';")
            .append("openPopup('").append("healthrecord/sendTransactionSelect.jsp&transactionId='+tranID+'&serverId='+serverID+'&ts=").append(getTs()).append("',500,500);\n")
            .append("}");
        ...
        boolean sendPDF = checkString(req.getParameter("sendPDF")).equals("true");
        if(sendPDF){
            html.append("window.setTimeout('doReferral();',500);\n");
        }
...
<%
    ...
    String sCSSNAV			 = "<link href='"+sCONTEXTPATH+"/_common/_css/navmain.css' rel='stylesheet' type='text/css'>";
    String sCSSJQUERY		 = "<link href='"+sCONTEXTPATH+"/_common/_css/jquery-ui.css' rel='stylesheet' type='text/css'>";
    ...
    String sJSJUIST = "<script>rcts = Juist.getClientRects();Juist.style.height = Math.floor(document.body.offsetHeight - rcts[0].top - 5);</script>";
    ...
    String sJSFULLCALENDAR = 	"<script src='"+sCONTEXTPATH+"/_common/_script/fullcalendar/core/main.js'></script>"+
			"<script src='"+sCONTEXTPATH+"/_common/_script/fullcalendar/interaction/main.js'></script>"+
			"<script src='"+sCONTEXTPATH+"/_common/_script/fullcalendar/daygrid/main.js'></script>"+
			"<script src='"+sCONTEXTPATH+"/_common/_script/fullcalendar/bootstrap/main.js'></script>"+
			"<script src='"+sCONTEXTPATH+"/_common/_script/fullcalendar/timeline/main.js'></script>"+
			"<script src='"+sCONTEXTPATH+"/_common/_script/fullcalendar/resource-common/main.js'></script>"+
			"<script src='"+sCONTEXTPATH+"/_common/_script/fullcalendar/resource-timeline/main.js'></script>"+
			"<script src='"+sCONTEXTPATH+"/_common/_script/fullcalendar/timegrid/main.js'></script>";
	String sCSSFULLCALENDAR= 	"<link href='"+sCONTEXTPATH+"/_common/_script/fullcalendar/core/main.css' rel='stylesheet' type='text/css'/>"+
			"<link href='"+sCONTEXTPATH+"/_common/_script/fullcalendar/daygrid/main.css' rel='stylesheet' type='text/css'/>"+
			"<link href='"+sCONTEXTPATH+"/_common/_script/fullcalendar/bootstrap/main.css' rel='stylesheet' type='text/css'/>"+
			"<link href='"+sCONTEXTPATH+"/_common/_script/fullcalendar/timegrid/main.css' rel='stylesheet' type='text/css'/>"+
			"<link href='"+sCONTEXTPATH+"/_common/_script/fullcalendar/timeline/main.css' rel='stylesheet' type='text/css'/>"+
			"<link href='"+sCONTEXTPATH+"/_common/_script/fullcalendar/resource-timeline/main.css' rel='stylesheet' type='text/css'/>";
