...
<%
    ...
    for(int i=0; i<userParameters.size(); i++){
    	...
    	menuElem = getMenuElement(session,sLongLabelId);
       	...
       	if(okToDisplayIcon){
            %><img style='width:16px;height:16px;cursor:pointer;padding: 0px 2px 0px;vertical-align: top' src="<%=sCONTEXTPATH%>/_img/shortcutIcons/<%=sIconName%>" onClick="<%=sIconOnClick%>" title="<%=sIconText%>"/>&nbsp;<%
       	}
    } 
    ...
    if(activePatient!=null && "1".equalsIgnoreCase((String)activePatient.adminextends.get("vip")) && !activeUser.getAccessRight("vipaccess.select")){
        ...
        if(sTmpPersonid.length() > 0){
            ...
            if(!bFingerPrint){
                %> <img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' onclick="enrollFingerPrint();" border="0" height='16px' src="<c:url value='/_img/icons/mobile/newfingerprint.png'/>" title="<%=getTranNoLink("web","enrollFingerPrint",sWebLanguage)%>"/><%
            }
            if(!bBarcode){
                %> <img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' onclick="printPatientCard();"  border='0' height='16px' src="<c:url value='/_img/icons/mobile/badge.png'/>" title="<%=getTranNoLink("web","printPatientCard",sWebLanguage)%>"/><%
            }
            if(!bPicture){
        ...
        <img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' onclick="initBarcode2();" border='0' height='16px' src="<c:url value='/_img/icons/mobile/qr.png'/>" title="<%=getTranNoLink("web","barcode",sWebLanguage)%>"/>
        <%
			if(activePatient!=null && activePatient.personid!=null && activePatient.personid.length()>0){
				if(MedwanQuery.getInstance().getConfigInt("enableImageHub",0)==1 && activePatient!=null && activePatient.comment5.length()>0){
                    %><img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' onclick="openImageHub()" border='0' height='16px' src="<c:url value='/_img/icons/icon_imagehub.png'/>" title="<%=getTranNoLink("web","imagehub",sWebLanguage)%>"/><%
				}
				else{
                    %><img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' onclick="showPACS()" border='0' height='16px' src="<c:url value='/_img/icons/mobile/xray2.png'/>" title="<%=getTranNoLink("web","imagestore",sWebLanguage)%>"/><%
				}
		        if(activePatient!=null && activeUser.getAccessRight("labos.patientlaboresults.select") && activePatient.hasLabRequests()){
		            %><input type='button' accesskey=':' onclick="clickMenuItem('javascript:searchLab();');" style='display: none'/><img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' onclick="clickMenuItem('javascript:searchLab();');" title="<%=getTranNoLink("Web","labresults",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/results.png'/>"/><%
		        }
        ...
        if(bMenu && activeUser.getAccessRight("labos.fastresults.select")){
            %><img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' onclick="clickMenuItem('javascript:showLabResultsEdit();');" title="<%=getTranNoLink("Web","fastresultsedit",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/lab.png'/>"/><%
        }
        if(bMenu && activeUser.getAccessRight("patient.administration.select")){
            %><img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' onclick="clickMenuItem('javascript:showAdminPopup();');" style="padding:2px" title="<%=getTranNoLink("Web","Administration",sWebLanguage)%>" height='16^px' src="<c:url value='/_img/icons/mobile/administration.png'/>"/><%
        }
    
        %><img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' onclick="clickMenuItem('javascript:searchMyHospitalized();');" title="<%=getTranNoLink("Web","my_hospitalized_patients",sWebLanguage)%>" title="<%=getTranNoLink("Web","my_hospitalized_patients",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/bed.png'/>"/><%
  
        if(activeUser.getAccessRight("manage.meals.select")){
            %>&nbsp;<img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' onclick="clickMenuItem('<c:url value="/main.do" />?Page=meals/index.jsp&ts='+new Date().getTime());" title="<%=getTranNoLink("web","meals",sWebLanguage)%>" title="<%=getTranNoLink("web","meals",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/meal.png'/>"/><%
        }
        
        %>
            <img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' onclick="clickMenuItem('javascript:searchMyVisits();');" title="<%=getTranNoLink("Web","my_on_visit_patients",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/doctor.png'/>"/>
            <img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' onclick="doPrint();" title="<%=getTranNoLink("Web","Print",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/print.png'/>" />
		<%
			if(MedwanQuery.getInstance().getConfigInt("calendarVersion",1)==1){
		%>
            <img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' id="ddIconAgenda" onclick="clickMenuItem('<c:url value='/main.do'/>?Page=planning/findPlanning.jsp&ts='+new Date().getTime());" title="<%=getTranNoLink("Web","Planning",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/calendar.png'/>"/>
        <%
			}else if(MedwanQuery.getInstance().getConfigInt("calendarVersion",1)==2){
    	%>
            <img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' id="ddIconAgenda" onclick="clickMenuItem('<c:url value='/main.jsp'/>?Page=calendar/calendar.jsp&ts='+new Date().getTime());" title="<%=getTranNoLink("Web","Planning",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/calendar.png'/>"/>
        <%
        	}
        
        String sHelp = MedwanQuery.getInstance().getConfigString("HelpFile");
        if(sHelp.length() > 0){
            %><img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' id="ddIconHelp" height='16px' src="<c:url value='/_img/icons/mobile/question.png'/>" height="18" border="0" title="<%=getTranNoLink("Web","help",sWebLanguage)%>" onclick="showmanual();" onmouseover='this.style.cursor="hand"' onmouseout='this.style.cursor="default"'><%
        }
        
        %><img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' id="ddIconFingerprint" onclick="readFingerprint();" title="<%=getTranNoLink("Web","Read_fingerprint",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/fingerprint.png'/>"/><%
    }
%>

<img style='cursor:pointer;padding: 0px 2px 0px;vertical-align: top' id="ddIconLogoff" onclick="clickMenuItem('javascript:confirmLogout();');" title="<%=getTranNoLink("Web","LogOff",sWebLanguage)%>" height='16px' src="<c:url value='/_img/icons/mobile/logout.png'/>"/>
&nbsp;   