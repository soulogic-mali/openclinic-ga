...
<%
    ...
    if(sAction.length()==0 && activePatient!=null){
        sName = checkString(activePatient.lastname);
...
<table width="90%" onKeyDown='checkKeyDown(event);'>
    <form name="SF" method='post' action="<c:url value='/patientslist.do'/>?ts=<%=getTs()%>" id="SF">
        ...
        <tr>
            ...
            <td align="right" valign='middle' nowrap><%=getTran(request,"Web","DateOfBirth", sWebLanguage)%>&nbsp;
                <input class='<%=setFocus("DateOfBirth",sDefaultFocus)%>' type='text' name='findDateOfBirth' id='findDateOfBirth' value="<%=sDateOfBirth%>" size='17' OnBlur='checkDate(this)' maxlength='10'>
            </td>
            <td width="1%" nowrap>
                <img style='vertical-align: middle' height='16px' src="<%=sCONTEXTPATH%>/_img/icons/icon_age.png" onclick='setAge();'/>
            <%	
        ...
        <tr>
            ...
            <td align="right" nowrap><%=getTran(request,"Web", "personid", sWebLanguage)%>&nbsp;
                <input class='<%=setFocus("personid",sDefaultFocus)%>' type='text' style='text-transform:uppercase' name='findPersonID' value="<%=MedwanQuery.getInstance().getConfigInt("isMPIServer",0)==1?ScreenHelper.convertToUUID(sPersonID):sPersonID%>" size='17' onblur='limitLength(this);'>
            </td>
        </tr>
        <%-- row 3 --%>
        <tr>
            ...
            <td colspan='2' nowrap>
                ...
                <%
                    if(sUnit.length() > 0){
                        %><img style='vertical-align: middle' src="<c:url value='/_img/icons/icon_info.gif'/>" class="link" alt="<%=getTranNoLink("Web","Information",sWebLanguage)%>" onclick='searchInfoService(SF.findUnit)'/><%
                    }
                %>
...
function setAge(){
	var age = window.prompt('<%=getTranNoLink("web","age",sWebLanguage)%>')*1;
	if(age>0){
		<% if(MedwanQuery.getInstance().getConfigInt("approximateDateStartsOnFirstOfJanuary",1)==1){%>
			document.getElementById('findDateOfBirth').value='01/01/'+(new Date().getFullYear()-age).toFixed(0);
		<%}else{%>
		var day = new Date().getDate();
		var month = new Date().getMonth()+1;
		if((day+"").length<2){day="0"+day}
		if((month+"").length<2){month="0"+month}
		document.getElementById('findDateOfBirth').value=day+'/'+month+'/'+(new Date().getFullYear()-age);
		<%}%>
	}
}
