...
<%
	...
	// serverid specified
	if(checkString(request.getParameter("zoneid")).length() > 0){
    	sZoneId = request.getParameter("zoneid");
    	lValues = TimeGraph.getListZoneValueGraph(beginDate,endDate,sZoneId,parameterId,sWebLanguage,activeUser.userid);
   
        sTitle = HTMLEntities.htmlentities(sZoneId) + "<br />" + HTMLEntities.htmlentities(getTranNoLink("datacenterparameter", parameterId, sWebLanguage))+
                                          (request.getParameter("fullperiod")!=null?" <a href=\\\"javascript:simpleZoneValueGraph('"+sZoneId+"','"+parameterId+"')\\\">("+ScreenHelper.getTranNoLink("web", "lastyear", sWebLanguage)+")</a>":" <a href=\\\"javascript:simpleZoneValueGraphFull('"+sZoneId+"','"+parameterId+"')\\\">("+ScreenHelper.getTranNoLink("web", "fullperiod", sWebLanguage)+")</a>");
    }
	else if(checkString(request.getParameter("serverid")).length() > 0){
