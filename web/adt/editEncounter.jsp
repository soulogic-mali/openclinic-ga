...
<%
    ...
    if(sEditEncounterUID.trim().length()==0 && (activePatient.isDead()!=null && !activeUser.getAccessRightNoSA("deceasedpatient.add"))){
    	out.println(getTran(request,"web","nopermission.deadpatient",activeUser==null || activeUser.person==null?"en":activeUser.person.language));
    }
    else{
	...
	%>
	    ...
	    <table class='list' border='0' width='100%' cellspacing='1'>
	       ...
	       <tr id="Bed" style="visibility:visible;">
	        ...
	        <%
	            int accountedDays = 0;
	            if(checkString(sEditEncounterUID).split("\\.").length==2){
	            	accountedDays=Encounter.getAccountedAccomodationDays(sEditEncounterUID);
	            }
	            Vector accomodationDebets = new Vector();
	            if(checkString(sEditEncounterUID).split("\\.").length==2){
	            	accomodationDebets=Encounter.getAccountedAccomodations(sEditEncounterUID);
	            }
	        %>
	        <tr id="alreadyAccountedAccomodation"  style="visibility: <%=accomodationDebets.size()>0?"visible":"visible"%>;">
	            <td class="admin"><%=getTran(request,"web","alreadyaccountedaccomodation",sWebLanguage)%> </td><td class='admin2'> = <%=checkString(sEditEncounterUID).split("\\.").length==2?Encounter.getAccountedAccomodationDays(sEditEncounterUID):0%> <%=getTran(request,"web","days",sWebLanguage)%><%=tmpEncounter!=null && tmpEncounter.getDurationInDays()>0?" = <label id='d2'>"+(checkString(sEditEncounterUID).split("\\.").length==2?Encounter.getAccountedAccomodationDays(sEditEncounterUID):0)*100/tmpEncounter.getDurationInDays()+"</label>%":""%>
	            <BR/>
	            <%
	...
	<script>  
	  ...
	  function setTime(field){
		  ...
		  var utc = Date.UTC(dateObj.getFullYear(), dateObj.getMonth(), dateObj.getDate(), dateObj.getHours(), dateObj.getMinutes(), dateObj.getSeconds(), dateObj.getMilliseconds());
		  var tz = (utc - dateObj.getTime()) / (60 * 60 * 1000);
		  if(<%=MedwanQuery.getInstance().getConfigInt("timeZone",-99)%>>-99){
			  tz=<%=MedwanQuery.getInstance().getConfigInt("timeZone",-99)%>-tz;
		  }
		  else{
			  tz=0;
		  }
	    document.getElementsByName(field)[0].value = (new Date().getHours()+tz>=10?new Date().getHours()+tz:"0"+(new Date().getHours()+tz))+":"+
	    ...
	    if(EditEncounterForm.EditEncounterBegin.value.split("/").length==3){
	      if(isDate(EditEncounterForm.EditEncounterBegin.value.split("/")[0],EditEncounterForm.EditEncounterBegin.value.split("/")[1],EditEncounterForm.EditEncounterBegin.value.split("/")[2])){
		    ...
		    if(end>=begin){
	          ...
	          var accounted = <%=checkString(sEditEncounterUID).split("\\.").length==2?Encounter.getAccountedAccomodationDays(sEditEncounterUID):0%>;
