...
%>
<%=checkPermission(out,"labos.worklists","select",activeUser)%>
<script>
...
<%
    if(activeProfile.length() > 0){
        ...
        <%
            ...
            out.print("<td class='tscrollrow'><center>"+getTran(request,"web","lab.technicalvalidation",sWebLanguage)+"</center></td>");
            out.print("<td class='tscrollrow'><center>"+getTran(request,"web","comment",sWebLanguage)+"</center></td></tr>");
            ...
            for(int n=0; n<results.size(); n++){
                    ...
                    if(hContentColumns.contains(analysis.get("labcode"))){
	                    ...
	                    if (requestedLabAnalysis != null && requestedLabAnalysis.getFinalvalidation()==0) {
	                        ...
	                        if(labAnalysis.getEditor().equalsIgnoreCase("antivirogram")){
	                        	...
	                        	out.print("<td style='background-color: "+bgcolor+"'><center><input style='background: "+sColor+"' type='text' size='5' name='store." + labRequest.getServerid() + "." + labRequest.getTransactionid() + "." + requestedLabAnalysis.getAnalysisCode() + "' value='" + requestedLabAnalysis.getResultValue() + "' onchange='validateAlert(this,\""+requestedLabAnalysis.getAnalysisCode()+"\");'></center></td>");
	                        }
	                    } else {
                            String sColor = bgcolor;
	                        if(requestedLabAnalysis !=null){
	                            ...
	                            if(labAnalysis.getEditor().equalsIgnoreCase("antivirogram")){
	                                ...
	                                out.print("<td style='background-color: "+bgcolor+"'><b><center>"+"<br/>"+requestedLabAnalysis.getResultValue()+"</center></b></td>");
	                            }
	                        }
	                        else{
                ...
                out.print("<td style='background-color: "+bgcolor+";text-align: center'><input type='hidden' name='store."+labRequest.getServerid()+"."+labRequest.getTransactionid()+".technicalvalidation' value='"+(bTechnicallyValidated ? "1" : "0")+"'><input class='checkbox' type='checkbox' "+(bTechnicallyValidated ? "checked onclick='return false;'" : "onchange='if(this.checked){document.getElementsByName(\"store."+labRequest.getServerid()+"."+labRequest.getTransactionid()+".technicalvalidation\")[0].value=\"1\"}else{document.getElementsByName(\"store."+labRequest.getServerid()+"."+labRequest.getTransactionid()+".technicalvalidation\")[0].value=\"0\"}'")+"/></td>");
                out.print("<td style='background-color: "+bgcolor+";min-width: 100px;text-align: center'><input class='text' type='text' size='20' name='store."+labRequest.getServerid()+"."+labRequest.getTransactionid()+".comment'/></td>");
                out.print("</tr>");
            }
        %>
        </table>
