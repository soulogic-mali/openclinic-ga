	...
	for(int n=0;n<pathwayFiles.length;n++){
		...
		if(root.element("treatments")!=null){
			...
			while(iTreatments.hasNext()){
				...
				//Check if the treatment is applicatble to this patient
				if(checkString(treatment.attributeValue("minage")).length()==0||activePatient.getAgeInMonths()>=Integer.parseInt(treatment.attributeValue("minage"))){
					if(checkString(treatment.attributeValue("maxage")).length()==0||activePatient.getAgeInMonths()<=Integer.parseInt(treatment.attributeValue("maxage"))){
						...
						if(sptSigns.get(excludeSpt)==null){
							...
							if(bMatch){
								if(excludeSpt.startsWith("alert")){
									sExcludes.append("<tr><td class='admin2' colspan='3'><b>"+excludeSign+"</b></td></tr>");
								}
								else{
						...
						while(iPaths.hasNext()){
							...
							if(hMissing.size()>0){
								...
								while(iMissing.hasNext()){
									...
									sMissing+="<tr><td width='50%'><b>"+concept.text+"</b></td><td><select class='text' onchange='updatePathway()' name='concept."+sConceptId+"' id='concept."+sConceptId+"' onchange='updateAll(\"concept."+sConceptId+"\",this.value)'><option/>";
